...
def run_simulation(pump_parameters,seed_parameters, sigmas_pump, sigmas_seed, notch_settings=None):
    ...
    max_number_single_passes = seed_parameters["max_number_single_passes"]


    N_seed_slices = seed_parameters["N_seed_slices"]
    seed_pulse_duration = seed_parameters["seed_pulse_duration"]

    p_inv_out_seed = np.zeros((max_number_single_passes))
    p_inv_out_pump = np.zeros((max_number_single_passes+1))
    E_pulse_energy = np.zeros((max_number_single_passes))
    J_spectrum_after_each_single_pass = np.zeros((max_number_single_passes, J_pulse_in.shape[0]))
    J_pulse_out = np.zeros((J_pulse_in.shape[0],max_number_single_passes))
    ...
    if (notch_settings == None):
        ...
        if (notch_settings["central_wavelength"]<sigmas_pump[-1,0] and notch_settings["central_wavelength"]>sigmas_pump[0,0]):
            ...
            notch_pump = np.ones(spectral_pump_fluence.shape[0])
            notch_seed = np.ones(J_pulse_in.shape[0])
    p_inv_out_pump[0],J_pulse_out_pump = sub_function_slice_fluence(spectral_pump_fluence,N_pump_slices, pump_time, p_inv_start, sigmas_pump, tau_gain, h,c,N_gain_ion_density,length_crystal, T_losses, notch_pump)
    ...
    while(not(stop_iterating)):
        i = number_passes
        p_inv_out_seed[i], J_pulse_out[:,i] = sub_function_slice_fluence(J_pulse_in,N_seed_slices, seed_pulse_duration, p_inv_out_pump[i], sigmas_seed, tau_gain, h,c,N_gain_ion_density,length_crystal, T_losses, notch_seed)
        ...
        if (saturation_condition):
            stop_iterating = True
        elif (number_passes >= max_number_single_passes-1):
            stop_iterating = True
        else:
            stop_iterating = False
